uploadBlob = function(action) {
	...
	if (blob !== '' && titlePaste !== '') {
		...
		Meteor.call("addPaste", blob, titlePaste, langPaste, getCookie("auth"), Session.get("isPasteEncrypted"), Session.get("isFork"), Session.get("pasteName"), Session.get("isBookmark"), function (err, response) {
...
deletePaste = function(pasteId) {
	Meteor.call("deletePaste", pasteId, Session.get("auth"), function(err,response){
		if(err) {console.log(err);}
		else {startToast(2000, "This paste has been deleted", "Deleted");}
		loadUserPastes();
...
deleteBookmark = function(bookmarkId) {
	Meteor.call("deleteBookmark", bookmarkId, Session.get("auth"), function(err,response){
		if(err) {console.log(err);}
		else {startToast(2000, "This bookmark has been deleted", "Deleted");}
		loadUserBookmarks();
	});
}
