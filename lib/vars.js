uploadBlob = function(action) {
	...
	if (blob !== '' && titlePaste !== '') {
		...
		Meteor.call("addPaste", blob, titlePaste, langPaste, getCookie("auth"), Session.get("isPasteEncrypted"), Session.get("isFork"), Session.get("isSave"), function (err, response) {
